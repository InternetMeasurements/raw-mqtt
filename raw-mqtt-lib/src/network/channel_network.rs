...
use crate::network::channel::{BoundedReceiver, BoundedSender, ChannelReceiver, ChannelResult, ChannelSender, SingleLifoQueue, UnboundedReceiver, UnboundedSender};
...
fn spawn_sender(tracker: TaskTracker, token: CancellationToken, mut from_producer: ChannelReceiver, mut tx_stream: impl AsyncWriteExt + Unpin + Send + 'static) {
...
fn spawn_receiver(tracker: TaskTracker, token: CancellationToken, to_consumer: async_channel::Sender<BytesMut>, mut rx_stream: impl AsyncReadExt + Unpin + Send + 'static) {
    tracker.spawn(async move {
...
impl Network for ChannelNetwork {
    ...
    async fn connect(&mut self, host: &String, port: &String, server_name: &String) -> Result<(), Box<dyn Error>> {
        ...
        if self.queue == -1 { // Unbounded FIFO queue
        ...
        match self.transport {
            Transport::TCP => {
                ...
                // Sender task
                spawn_sender(self.tracker.clone(), self.cancellation_token.clone(), from_producer, tcp.tx_stream);

                // Receiver task
                spawn_receiver(self.tracker.clone(), self.cancellation_token.clone(), to_consumer, tcp.rx_stream);
            },
            Transport::TLS => {
                let tls = Tls::new(host, port, &self.insecure, server_name).await?;

                // Sender task
                spawn_sender(self.tracker.clone(), self.cancellation_token.clone(), from_producer, tls.tx_stream);

                // Receiver task
                spawn_receiver(self.tracker.clone(), self.cancellation_token.clone(), to_consumer, tls.rx_stream);
            },
            Transport::QUIC => {
                let quic = Quic::new(host, port, &self.insecure, &server_name).await?;

                // Sender task
                spawn_sender(self.tracker.clone(), self.cancellation_token.clone(), from_producer, quic.tx_stream);

                // Receiver task
                spawn_receiver(self.tracker.clone(), self.cancellation_token.clone(), to_consumer, quic.rx_stream);
            }
        };
        ...
        match self.to_sender {
            Some(ref mut tx_stream) => {
                ...
                match res {
                    ...
                    Ok(ChannelResult::Replaced) => {
                        Err("Replaced into queue")?
                    },
                    Err(_) => {
                        Err("Failed to insert in queue")?
                    }
                }
            },
            None => {
